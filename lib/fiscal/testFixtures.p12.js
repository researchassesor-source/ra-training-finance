...
export function buildTestCertificate({
  ...
  // Orden "raíz a hoja" (C, O, CN) -- así codifica el DN un certificado X.509 real
  // (ver el Issuer DER de cualquier CA acreditada), y así lo expone
  // certificate.issuer.attributes en node-forge. Deliberadamente NO en orden CN
  // primero: con eso, buildIssuerSerial (xadesSign.js) habría necesitado su
  // `.reverse()` sin que ningún test lo notara. El valor de CN incluye una coma a
  // propósito, para ejercer el escapado RFC 2253 en xadesSign.test.js.
  cert.setIssuer([
    { name: 'countryName', value: 'EC' },
