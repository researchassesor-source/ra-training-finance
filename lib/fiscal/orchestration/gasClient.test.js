...
import { callGasAction, callGasActionAsUser, callGasLogin, GasClientError } from './gasClient.js'
...
describe('callGasAction — payload real enviado a Apps Script', () => {
  ...
  it('callGasLogin usa el endpoint login real para emitir una sesión humana fresca y no envía serviceToken', async () => {
    const { fetchImpl, llamadas } = fetchQueCapturaElBody({ success: true, token: 'tok_fresco', user: { rol: 'admin' } })
    const resultado = await callGasLogin('admin', 'password-temporal-no-logueada', { gasUrl: 'https://script.google.com/macros/s/fixture/exec', fetchImpl })

    expect(resultado.token).toBe('tok_fresco')
    expect(llamadas).toHaveLength(1)
    expect(llamadas[0].body.action).toBe('login')
    expect(llamadas[0].body.username).toBe('admin')
    expect('serviceToken' in llamadas[0].body).toBe(false)
    expect('token' in llamadas[0].body).toBe(false)
  })

  it('callGasLogin rechaza credenciales inválidas sin exponer la contraseña en el error', async () => {
    const password = 'password-temporal-no-logueada'
    const { fetchImpl } = fetchQueCapturaElBody({ success: false, error: 'Usuario o contraseña incorrectos.' })

    let mensaje = ''
    try {
      await callGasLogin('admin', password, { gasUrl: 'https://script.google.com/macros/s/fixture/exec', fetchImpl })
    } catch (err) {
      mensaje = err.message
    }
    expect(mensaje).toContain('Usuario o contraseña incorrectos.')
