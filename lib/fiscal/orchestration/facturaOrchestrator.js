...
import { buildRidePdfBytes, sha256Hex as sha256RideHex } from '../ridePdf.js'

export class OrchestratorError extends Error {}

const RESOLVED_AUTHORIZATION_STATES = new Set(['AUTHORIZED', 'NOT_AUTHORIZED', 'DELIVERY_PENDING', 'DELIVERED'])
const VALID_SRI_PAYMENT_CODES = new Set(['01', '15', '16', '17', '18', '19', '20', '21'])
...
function assertSriPaymentCodeForXml(factura) {
  const code = String(factura?.SriPaymentCode || '').trim()
  if (code && !VALID_SRI_PAYMENT_CODES.has(code)) {
    throw new OrchestratorError(`SriPaymentCode no reconocido para ${factura.ID || 'factura'}: ${code}`)
  }
  if (factura?.Environment === 'production' && !code) {
    throw new OrchestratorError(`La factura productiva ${factura.ID || ''} no tiene SriPaymentCode. Se bloquea la generación/transmisión para evitar XML fiscal incompleto.`)
  }
  return code
}
...
export function buildXmlParaFactura(factura, items, claveAcceso, emisor) {
  const sriPaymentCode = assertSriPaymentCodeForXml(factura)
  ...
  return buildFacturaXml({
    ...
    impuestosTotales,
    pagos: sriPaymentCode ? [{ formaPago: sriPaymentCode, totalCents: Number(factura.GrandTotal) }] : [],
...
/**
 * Cierre posterior a autorización: genera el RIDE localmente, lo persiste en el
 * backend como artefacto privado y solo entonces marca la factura como DELIVERED.
 * No llama al SRI, no cambia clave/secuencial/XML autorizado y es idempotente.
 */
export async function finalizarEntregaFiscal(facturaId, options) {
  const { gasOptions } = options
  const { factura, items } = await callGasAction('getFacturaFiscalCompleta', { facturaId }, gasOptions)

  if (factura.SriAuthorizationStatus !== 'AUTORIZADO' || !factura.AuthorizationNumber) {
    throw new OrchestratorError(`La factura ${facturaId} no tiene autorización SRI completa.`)
  }
  if (!['DELIVERY_PENDING', 'DELIVERED'].includes(factura.Status)) {
    return { outcome: 'NOT_READY', status: factura.Status }
  }
  if (!factura.XmlAuthorizedContent && !factura.XmlAuthorizedReference) {
    throw new OrchestratorError(`La factura ${facturaId} no tiene XML autorizado recuperable.`)
  }

  if (factura.RideReference && factura.Sha256Ride) {
    const closed = await callGasAction('cerrarEntregaFiscal', {
      facturaId,
      motivo: 'RIDE y XML autorizado ya disponibles; cierre idempotente.',
    }, gasOptions)

    return {
      outcome: 'DELIVERED',
      factura: closed.factura,
      rideReference: factura.RideReference,
      sha256Ride: factura.Sha256Ride,
      rideIdempotent: true,
      closeIdempotent: Boolean(closed.idempotent),
    }
  }

  const ridePdfBytes = buildRidePdfBytes(factura, items)
  const sha256Ride = sha256RideHex(ridePdfBytes)
  const ridePdfBase64 = Buffer.from(ridePdfBytes).toString('base64')
  const filename = `RIDE_${String(factura.DocumentNumber || factura.ID).replace(/[\\/:*?"<>|]+/g, '_')}.pdf`

  const stored = await callGasAction('guardarRideFiscal', {
    facturaId,
    ridePdfBase64,
    sha256Ride,
    filename,
  ...
  const closed = await callGasAction('cerrarEntregaFiscal', {
    facturaId,
    motivo: 'RIDE y XML autorizado disponibles para descarga controlada.',
  }, gasOptions)

  return {
    outcome: 'DELIVERED',
    factura: closed.factura,
    rideReference: stored.rideReference,
    sha256Ride: stored.sha256Ride,
    rideIdempotent: Boolean(stored.idempotent),
    closeIdempotent: Boolean(closed.idempotent),
  }
}
