...
import { generarYFirmarFactura, enviarFacturaARecepcion, consultarYActualizarAutorizacion, continuarFlujoFactura, reconciliarSubmittingEnvejecido } from './facturaOrchestrator.js'
...
describe('reabrirFacturaRechazadaParaCorreccion — reapertura explícita de NOT_AUTHORIZED', () => {
  ...
  it('el script CLI TEST_ONLY exige environment=test, la FacturaID exacta y rechazo previo antes de reabrir, y usa FISCAL_USER_TOKEN (no serviceToken)', () => {
    ...
    expect(scriptSource).toContain('FISCAL_USER_TOKEN')
    // La llamada de reapertura usa callGasActionAsUser(accion, params, userToken, ...)
    // -- userToken (de FISCAL_USER_TOKEN) como credencial, nunca gasOptions/serviceToken.
