...
import { generarYFirmarFactura, enviarFacturaARecepcion, consultarYActualizarAutorizacion, continuarFlujoFactura } from './facturaOrchestrator.js'
...
describe('enviarFacturaARecepcion', () => {
  ...
  it('timeout ANTES de completar Recepción libera el claim (vuelve a SIGNED) y sube RetryCount, sin marcar RECEIVED', async () => {
    const harness = seededHarness()
    const { facturaId, signedXml, gasOptions } = await facturaFirmada(harness)
    ...
    expect(Number(f.RetryCount)).toBe(1)
  })

  it('error de red se comporta igual que timeout: libera el claim para reintentar', async () => {
    const harness = seededHarness()
    const { facturaId, signedXml, gasOptions } = await facturaFirmada(harness)
  ...
  it('SOAP Fault libera el claim igual que un timeout', async () => {
    const harness = seededHarness()
    const { facturaId, signedXml, gasOptions } = await facturaFirmada(harness)
...
describe('continuarFlujoFactura — recuperación tras reinicio', () => {
  ...
  it('una factura atascada en SUBMITTING no se reintenta automáticamente: exige revisión manual', async () => {
    const harness = seededHarness()
    const facturaId = crearYReservar(harness)
    const gasOptions = gasHarnessOptions(harness)
    await generarYFirmarFactura(facturaId, { environment: 'test', emisor: EMISOR, signingKeys: testSigningKeys(), gasOptions })
    ...
    expect(result.outcome).toBe('REQUIRES_MANUAL_REVIEW')
