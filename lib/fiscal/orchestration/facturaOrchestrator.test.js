...
describe('continuarFlujoFactura — recuperación tras reinicio', () => {
  it('retoma desde SEQUENCE_RESERVED (proceso murió antes de generar la clave de acceso) y completa hasta RECEIVED', async () => {
    const harness = seededHarness()
    const facturaId = crearYReservar(harness)
    const gasOptions = gasHarnessOptions(harness)
    expect(facturaActual(harness, facturaId).Status).toBe('SEQUENCE_RESERVED')

    const result = await continuarFlujoFactura(facturaId, {
      environment: 'test', emisor: EMISOR, signingKeys: testSigningKeys(), gasOptions,
      sriOptions: { fetchImpl: fakeFetch(FIXTURE_RECIBIDA) },
    })
    expect(result.outcome).toBe('RECEIVED')
    expect(facturaActual(harness, facturaId).AccessKey).toBeTruthy()
  })

  it('retoma desde GENERATED (la firma falló después de persistir la clave de acceso — mismo patrón que dejó una factura real "Preparando" tras el HTTP 502 de canonicalización) reutilizando la MISMA clave de acceso, sin generar otra', async () => {
    const harness = seededHarness()
    const facturaId = crearYReservar(harness)
    const gasOptions = gasHarnessOptions(harness)

    // Simula el incidente real: la clave de acceso ya quedó persistida (GENERATED) pero
    // la firma falla (aquí, llaves de firma deliberadamente inválidas) antes de llegar a
    // SIGNED -- el mismo patrón que dejó una factura real en "Preparando".
    const llavesRotas = { ...testSigningKeys(), privateKeyPem: 'no-es-una-llave-privada-valida' }
    await expect(generarYFirmarFactura(facturaId, { environment: 'test', emisor: EMISOR, signingKeys: llavesRotas, gasOptions }))
      .rejects.toThrow()
    expect(facturaActual(harness, facturaId).Status).toBe('GENERATED')
    const claveGenerada = facturaActual(harness, facturaId).AccessKey
    expect(claveGenerada).toBeTruthy()

    // La "siguiente ejecución" (con las llaves reales) retoma desde GENERATED vía
    // continuarFlujoFactura y reutiliza la MISMA clave -- nunca reserva ni genera otra.
    const result = await continuarFlujoFactura(facturaId, {
      environment: 'test', emisor: EMISOR, signingKeys: testSigningKeys(), gasOptions,
      sriOptions: { fetchImpl: fakeFetch(FIXTURE_RECIBIDA) },
    })
    expect(result.outcome).toBe('RECEIVED')
    expect(facturaActual(harness, facturaId).AccessKey).toBe(claveGenerada)
  })
