 ...
 * - Canonicalización C14N: se delega a `xmllint --c14n` (libxml2), el motor de
 *   referencia ya usado para la validación XSD — evita una segunda implementación de
 *   C14N (fuente típica de bugs sutiles de interoperabilidad) dentro de este repo.
...
/** Canonicaliza un fragmento XML bien formado con W3C C14N 1.0 (no exclusiva) vía xmllint. */
export function canonicalizeC14n(xmlFragment) {
  ...
  try {
    ...
    return execFileSync('xmllint', ['--c14n', filePath], { stdio: ['ignore', 'pipe', 'pipe'] })
  } catch (err) {
    throw new XadesSignError(`No se pudo canonicalizar el fragmento XML (xmllint --c14n): ${err.message}`)
  } finally {
...
function buildIssuerSerial(certificate) {
  const issuerName = certificate.issuer.attributes.map(a => `${a.shortName}=${a.value}`).join(',')
...
export function signFacturaXml(unsignedXml, { privateKeyPem, certificatePem, certificateBase64, certificate }, options = {}) {
  ...
  const signatureXml =
    `<ds:Signature xmlns:ds="${NS_DS}" xmlns:etsi="${NS_ETSI}" Id="${sigId}">` +
    signedInfoXml +
