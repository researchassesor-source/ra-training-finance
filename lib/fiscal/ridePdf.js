...
function paymentLabel(factura) {
  const internal = text(factura.PaymentMethodInternal, '')
  const code = text(factura.SriPaymentCode, '')
  if (internal && code) return `${internal} (SRI ${code})`
  if (code) return `Codigo SRI ${code}`
  return internal || '-'
}
...
export function buildRidePdfBytes(factura, items = []) {
  ...
  doc.setFontSize(11)
  doc.text('RIDE - FACTURA ELECTRONICA', 130, 16)

  doc.setDrawColor(20, 64, 120)
  doc.rect(12, 22, 186, 34)
  doc.setFontSize(9)
  addLabel(doc, 'RUC EMISOR', factura.IssuerRuc, 16, 30, 60)
  addLabel(doc, 'RAZON SOCIAL', 'RESEARCH ASSESSOR TRAINING S.A.S.', 16, 43, 90)
  addLabel(doc, 'NUMERO', factura.DocumentNumber || `${factura.Establishment}-${factura.EmissionPoint}-${factura.Sequential}`, 125, 30, 60)
  addLabel(doc, 'AMBIENTE', factura.Environment === 'production' ? 'PRODUCCION' : 'PRUEBAS', 125, 43, 60)

  doc.rect(12, 60, 186, 42)
  addLabel(doc, 'CLAVE DE ACCESO', factura.AccessKey, 16, 68, 170)
  addLabel(doc, 'AUTORIZACION', factura.AuthorizationNumber, 16, 82, 170)
  addLabel(doc, 'FECHA AUTORIZACION', factura.AuthorizationDate, 125, 82, 60)
  addLabel(doc, 'FECHA EMISION', factura.IssueDate, 16, 95, 60)
  addLabel(doc, 'FORMA DE PAGO', paymentLabel(factura), 125, 95, 60)

  doc.rect(12, 106, 186, 34)
  addLabel(doc, 'COMPRADOR', factura.BuyerName, 16, 114, 105)
  addLabel(doc, 'IDENTIFICACION', factura.BuyerIdentification, 125, 114, 60)
  addLabel(doc, 'EMAIL', factura.BuyerEmail, 16, 128, 80)
  addLabel(doc, 'DIRECCION', factura.BuyerAddress, 100, 128, 90)

  const startY = 150
  doc.setFillColor(20, 64, 120)
  doc.setTextColor(255, 255, 255)
  doc.rect(12, startY, 186, 8, 'F')
  doc.text('Codigo', 16, startY + 5.5)
  doc.text('Descripcion', 42, startY + 5.5)
  ...
  doc.setFontSize(8)
  doc.text('Representacion impresa de documento electronico autorizado por el SRI.', 14, 282)
