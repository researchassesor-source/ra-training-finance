...
describe('signFacturaXml — estructura XAdES-BES', () => {
  ...
  it('no declara etsi en ds:Signature para evitar que C14N inclusivo contamine SignedInfo/KeyInfo con namespaces heredados', () => {
    const unsignedXml = buildFacturaXml(facturaPruebaSri())
    const signedXml = signFacturaXml(unsignedXml, testSigningKeys({ password: TEST_PASSWORD }))
    const signatureStart = signedXml.match(/<ds:Signature[^>]+>/)?.[0] || ''
    expect(signatureStart).toContain('xmlns:ds="http://www.w3.org/2000/09/xmldsig#"')
    expect(signatureStart).not.toContain('xmlns:etsi=')
    expect(signedXml).toContain('<etsi:QualifyingProperties xmlns:etsi="http://uri.etsi.org/01903/v1.3.2#"')
  })
