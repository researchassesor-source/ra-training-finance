...
export function getLocalFiscalDemoUser({ isDev = import.meta.env.DEV, enabled = sriBillingEnabled, demoAuth = import.meta.env.VITE_LOCAL_FISCAL_DEMO_AUTH === 'true' } = {}) {
  if (!isDev || !enabled || !demoAuth) return null
...
export function nextFiscalAction(document) {
  ...
  if (document.status === 'READY_TO_SIGN') return { key: 'sign', label: 'Aplicar firma mock' }
